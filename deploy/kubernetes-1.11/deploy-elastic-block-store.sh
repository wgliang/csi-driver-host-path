...
# deploy the elastic-block-store plugin driver.  This should be considered
...
# - CSI_SNAPSHOTTER_TAG
# - ELASTIC_BLOCK_STORE_PLUGIN_REGISTRY
# - ELASTIC_BLOCK_STORE_PLUGIN_TAG
#
...
CSI_PROVISIONER_RBAC_YAML="https://raw.githubusercontent.com/kubernetes-csi/external-provisioner/$(rbac_version "${BASE_DIR}/elastic-block-store/csi-elastic-block-store-provisioner.yaml" csi-provisioner false)/deploy/kubernetes/rbac.yaml"
: ${CSI_PROVISIONER_RBAC:=https://raw.githubusercontent.com/kubernetes-csi/external-provisioner/$(rbac_version "${BASE_DIR}/elastic-block-store/csi-elastic-block-store-provisioner.yaml" csi-provisioner "${UPDATE_RBAC_RULES}")/deploy/kubernetes/rbac.yaml}
CSI_ATTACHER_RBAC_YAML="https://raw.githubusercontent.com/kubernetes-csi/external-attacher/$(rbac_version "${BASE_DIR}/elastic-block-store/csi-elastic-block-store-attacher.yaml" csi-attacher false)/deploy/kubernetes/rbac.yaml"
: ${CSI_ATTACHER_RBAC:=https://raw.githubusercontent.com/kubernetes-csi/external-attacher/$(rbac_version "${BASE_DIR}/elastic-block-store/csi-elastic-block-store-attacher.yaml" csi-attacher "${UPDATE_RBAC_RULES}")/deploy/kubernetes/rbac.yaml}
CSI_SNAPSHOTTER_RBAC_YAML="https://raw.githubusercontent.com/kubernetes-csi/external-snapshotter/$(rbac_version "${BASE_DIR}/elastic-block-store/csi-elastic-block-store-snapshotter.yaml" csi-snapshotter false)/deploy/kubernetes/rbac.yaml"
: ${CSI_SNAPSHOTTER_RBAC:=https://raw.githubusercontent.com/kubernetes-csi/external-snapshotter/$(rbac_version "${BASE_DIR}/elastic-block-store/csi-elastic-block-store-snapshotter.yaml" csi-snapshotter "${UPDATE_RBAC_RULES}")/deploy/kubernetes/rbac.yaml}
...
# deploy elastic-block-store plugin and registrar sidecar
echo "deploying elastic-block-store components"
for i in $(ls ${BASE_DIR}/elastic-block-store/*.yaml | sort); do
    echo "   $i"
...
# snapshotter, socat and elastic-block-store plugin in the default namespace.
cnt=0
while [ $(kubectl get pods 2>/dev/null | grep '^csi-elastic-block-store.* Running ' | wc -l) -lt 5 ] || ! kubectl describe volumesnapshotclasses.snapshot.storage.k8s.io 2>/dev/null >/dev/null; do
    if [ $cnt -gt 30 ]; then
        echo "Running pods:"
        kubectl describe pods

        echo >&2 "ERROR: elastic-block-store deployment not ready after over 5min"
        exit 1
    fi
    echo $(date +%H:%M:%S) "waiting for elastic-block-store deployment to complete, attempt #$cnt"
    cnt=$(($cnt + 1))
    sleep 10
done
...
kubectl apply -f ${BASE_DIR}/snapshotter/csi-elastic-block-store-snapshotclass.yaml
